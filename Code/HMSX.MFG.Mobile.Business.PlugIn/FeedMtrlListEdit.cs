    ...
    public class FeedMtrlListEdit: AbstractMobilePlugin
    {
       ...
       string MoBillNo = "";
        ...
        public override void OnInitialize(InitializeEventArgs e)
        {
            ...
            //获取已领料数据
            string strSql = string.Format(@"SELECT T1.FPRDORGID,T1.FMOBillNO,T1.FMOENTRYSEQ,T2.FSEQ,T2.FID,T2.FENTRYID,T2.FMATERIALID,T4.FNUMBER,T5.FNAME,T5.FSPECIFICATION,T2.FBASEUNITID,T3.FSTOCKID,T.FPickQty,T.FPgEntryId,PA.FACTUALQTY,PA.FLOT,PA.FLOT_TEXT FROM t_PgBomInfo T
            ...
            if (scanText != "")
            {
                ...
                foreach (DynamicObject obj in ppBomInfo)
                {
                    if (obj["FNUMBER"].ToString() == materialNumber && obj["FLOT_TEXT"].ToString() == lot_txt)
                    {
                        ...
                        feedinfo.lot = obj["FLOT_TEXT"].ToString();
